...
def generate_trips(clean_tips, edges, vehicles):
    ...
    pl.when(pl.col("class.type") == "Road")
      .then(pl.col("route").str.split(" ") # split route string
            ...
            .map_elements(lambda link_list: None if link_list is None
                          else [matsim_to_metro_links.get(link) for link in link_list[1:]], # ignore the first edge, else metropolis will start at the source node
