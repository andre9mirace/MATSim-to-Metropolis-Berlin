...
# In case you saved the plans' contents in another directory
...
persons = pl.read_parquet(os.path.join(MATSIM_TRIPS + "MATSim_persons.parquet"))    # person_id and agent characteristics (economic status, employment, vehicles, etc.)
plans = pl.read_parquet(os.path.join(MATSIM_TRIPS + "MATSim_plans.parquet"))   # plan_id with assigned person_id and score
activities = pl.read_parquet(os.path.join(MATSIM_TRIPS + "MATSim_activities.parquet")) # activities (place, duration, etc.) with associated to their respective plan
legs = pl.read_parquet(os.path.join(MATSIM_TRIPS + "MATSim_legs.parquet"))     # legs associated to a plan (with mode, departure times and routing mode)
routes = pl.read_parquet(os.path.join(MATSIM_TRIPS + "MATSim_routes.parquet"))     # routes, travel times, distances and travel times associated to a leg
